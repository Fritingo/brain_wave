...
def zeroto512(filename,file_num,Demarcation_point,off_with_head):
    ...
    for record_num in range(0,file_num,1):#record_file_num
        ...
        print('record_1d_arr shape:',np.shape(record_1d_arr))
        record_3d = record_1d_arr.reshape(-1, 512, 16)#3d_array
        print('record_3d shape:',np.shape(record_3d))
...
if __name__ == "__main__":
    ...
    print('dataset_done shape:',np.shape(dataset_done))
    print('dataset_done Max:',np.amax(dataset_done), 'dataset_done Min:',np.amin(dataset_done))
 # Parameters setting
    batch_size = 16
    epoch = 100
    time_step = 512
